...
public class BTreeNode {
	...
	public BTreeNode(BTree baum, BTreeNode vater, int ordnung) {
	...
	public Integer suchen(Integer key) {
		...
		return kind.suchen(key); // startet rekursive Suche und gibt Wert zur�ck
	} 

	public boolean  isKeyInKeys (Integer key){
	...
	/*
	  Einf�gen:  erst normal Einf�gen und dann splitten bis keine �berl�ufe */
	...
	int einfuegePosition(Integer key, Integer[] ls) {
		...
		while (i < ls.length) { // Durchlaufe die Werteliste
			...
			if(key < ls[i]){ // Vergleiche Key, falls kleiner gib die Position zur�ck
				return i;
			} else {
				i++; // Ansonsten weiter
			}
	...
	BTreeNode getTeilbaumFuer(Integer key) {
		int i = einfuegePosition(key, keys); // �berpr�fe die Einf�geposition 
